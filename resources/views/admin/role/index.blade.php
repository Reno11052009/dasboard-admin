            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex flex-wrap gap-1 max-w-md">
                            @if(is_array($role->permissions) && count($role->permissions) > 0)
                                ...
                                @else
                                    @foreach(array_slice($role->permissions, 0, 10) as $perm)
                                        <span class="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-700 rounded-lg">{{ $perm }}</span>
                                    @endforeach
                                    @if(count($role->permissions) > 10)
                                        <span class="px-2 py-1 text-xs font-medium bg-gray-100 text-gray-700 rounded-lg">+{{ count($role->permissions) - 10 }} lainnya</span>
                                    @endif
                                @endif
                            @else
                    ...
                    <td class="px-6 py-4 text-right space-x-2">
                        ...
                        @if($role->name !== 'Super Admin')
                        <form action="{{ route('roles.delete', $role->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-600 hover:bg-red-50 transition cursor-pointer" onclick="confirmDelete(event, this)" title="Hapus">
